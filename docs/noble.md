...
The project pins Node.js 24.19.0 in `.node-version` and supports Node.js 22 or
newer. The native fuzzer build also needs Bash, Clang with libFuzzer, CMake,
Make, Python 3, Git, npm, `curl`, and `tar`. The complete differential build
uses the pinned Go toolchain for its CIRCL post-quantum oracle.
...
The reusable workflow derives the adapter from the caller repository's exact
name, verifies that its Cryptofuzz module depends on the matching `@noble/*`
package, checks out and builds the latest commit on the caller's default
branch, and fuzzes only that adapter for two hours at 14:00 UTC. Each run
