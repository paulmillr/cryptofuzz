...
repository's default branch. The reusable workflow derives the adapter from
the repository name, fuzzes only that adapter for two hours with a fresh random
seed, and uploads the resulting corpus and crash artifacts.

The supported repository names are `noble-ciphers`, `noble-curves`,
`noble-ed25519`, `noble-hashes`, `noble-post-quantum`, and
`noble-secp256k1`. Pin `@master` to a full Cryptofuzz commit SHA if an immutable
workflow revision is preferred.
