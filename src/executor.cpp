...
/* Specializations for post-quantum KEM operations */
template<> void ExecutorBase<component::KEMKeyPair, operation::KEM_KeyGen>::postprocess(std::shared_ptr<Module> module, operation::KEM_KeyGen& op, const ExecutorBase<component::KEMKeyPair, operation::KEM_KeyGen>::ResultPair& result) const {
    (void)module;
    if ( result.second != std::nullopt ) {
        fuzzing::memory::memory_test_msan(result.second->publicKey.GetPtr(), result.second->publicKey.GetSize());
        fuzzing::memory::memory_test_msan(result.second->secretKey.GetPtr(), result.second->secretKey.GetSize());
        Pool_KEMKeyPair.Set({
            op.kemType.Get(),
            result.second->publicKey.ToHex(),
            result.second->secretKey.ToHex(),
        });
    }
}

template<> std::optional<component::KEMKeyPair> ExecutorBase<component::KEMKeyPair, operation::KEM_KeyGen>::callModule(std::shared_ptr<Module> module, operation::KEM_KeyGen& op) const {
    return module->OpKEM_KeyGen(op);
}

template<> void ExecutorBase<component::KEMEncapsulation, operation::KEM_Encapsulate>::postprocess(std::shared_ptr<Module> module, operation::KEM_Encapsulate& op, const ExecutorBase<component::KEMEncapsulation, operation::KEM_Encapsulate>::ResultPair& result) const {
    if ( result.second != std::nullopt ) {
        fuzzing::memory::memory_test_msan(result.second->ciphertext.GetPtr(), result.second->ciphertext.GetSize());
        fuzzing::memory::memory_test_msan(result.second->sharedSecret.GetPtr(), result.second->sharedSecret.GetSize());

        const auto publicKey = op.publicKey.ToHex();
        const auto keyPair = Pool_KEMKeyPair.Find([&](const KEMKeyPair_PoolEntry& entry) {
            return entry.kemID == op.kemType.Get() && entry.publicKey == publicKey;
        });
        if ( keyPair != std::nullopt ) {
            Pool_KEMEncapsulation.Set({
                op.kemType.Get(),
                keyPair->secretKey,
                result.second->ciphertext.ToHex(),
                result.second->sharedSecret.ToHex(),
            });

            operation::KEM_Decapsulate decapsulate(
                op.kemType,
                component::PQSecretKey(util::HexToBin(keyPair->secretKey)),
                result.second->ciphertext,
                component::Modifier());
            const auto decapsulated = module->OpKEM_Decapsulate(decapsulate);
            if ( decapsulated == std::nullopt || !(*decapsulated == result.second->sharedSecret) ) {
                printf("Cannot decapsulate KEM ciphertext\n\n");
                printf("Operation:\n%s\n", op.ToString().c_str());
                abort(
                    {module->name},
                    op.Name(),
                    op.GetAlgorithmString(),
                    "cannot decapsulate ciphertext");
            }
        }
    }
}

template<> std::optional<component::KEMEncapsulation> ExecutorBase<component::KEMEncapsulation, operation::KEM_Encapsulate>::callModule(std::shared_ptr<Module> module, operation::KEM_Encapsulate& op) const {
    return module->OpKEM_Encapsulate(op);
}

template<> void ExecutorBase<component::SharedSecret, operation::KEM_Decapsulate>::postprocess(std::shared_ptr<Module> module, operation::KEM_Decapsulate& op, const ExecutorBase<component::SharedSecret, operation::KEM_Decapsulate>::ResultPair& result) const {
    (void)module;
    (void)op;
    if ( result.second != std::nullopt ) {
        fuzzing::memory::memory_test_msan(result.second->GetPtr(), result.second->GetSize());
...
template<> std::optional<component::SharedSecret> ExecutorBase<component::SharedSecret, operation::KEM_Decapsulate>::callModule(std::shared_ptr<Module> module, operation::KEM_Decapsulate& op) const {
    return module->OpKEM_Decapsulate(op);
}

/* Specializations for post-quantum signature operations */
template<> void ExecutorBase<component::PQSignatureKeyPair, operation::PQSIG_KeyGen>::postprocess(std::shared_ptr<Module> module, operation::PQSIG_KeyGen& op, const ExecutorBase<component::PQSignatureKeyPair, operation::PQSIG_KeyGen>::ResultPair& result) const {
    (void)module;
    if ( result.second != std::nullopt ) {
        fuzzing::memory::memory_test_msan(result.second->publicKey.GetPtr(), result.second->publicKey.GetSize());
        fuzzing::memory::memory_test_msan(result.second->secretKey.GetPtr(), result.second->secretKey.GetSize());
        Pool_PQSignatureKeyPair.Set({
            op.pqSignatureType.Get(),
            result.second->publicKey.ToHex(),
            result.second->secretKey.ToHex(),
        });
    }
...
template<> void ExecutorBase<component::PQSignature, operation::PQSIG_Sign>::postprocess(std::shared_ptr<Module> module, operation::PQSIG_Sign& op, const ExecutorBase<component::PQSignature, operation::PQSIG_Sign>::ResultPair& result) const {
    if ( result.second != std::nullopt ) {
        fuzzing::memory::memory_test_msan(result.second->GetPtr(), result.second->GetSize());

        const auto secretKey = op.secretKey.ToHex();
        const auto keyPair = Pool_PQSignatureKeyPair.Find([&](const PQSignatureKeyPair_PoolEntry& entry) {
            return entry.pqSignatureID == op.pqSignatureType.Get() && entry.secretKey == secretKey;
        });
        if ( keyPair != std::nullopt ) {
            Pool_PQSignature.Set({
                op.pqSignatureType.Get(),
                keyPair->publicKey,
                op.message.ToHex(),
                result.second->ToHex(),
            ...
            operation::PQSIG_Verify verify(
                op.pqSignatureType,
                component::PQPublicKey(util::HexToBin(keyPair->publicKey)),
                op.message,
                *result.second,
                op.context,
                component::Modifier());
            const auto verified = module->OpPQSIG_Verify(verify);
            if ( verified == std::nullopt || *verified == false ) {
                printf("Cannot verify generated post-quantum signature\n\n");
                printf("Operation:\n%s\n", op.ToString().c_str());
                abort(
                    {module->name},
                    op.Name(),
...
template<> void ExecutorBase<bool, operation::PQSIG_Verify>::postprocess(std::shared_ptr<Module> module, operation::PQSIG_Verify& op, const ExecutorBase<bool, operation::PQSIG_Verify>::ResultPair& result) const {
    (void)module;
    (void)op;
    (void)result;
}
...
template class ExecutorBase<component::KEMKeyPair, operation::KEM_KeyGen>;
template class ExecutorBase<component::KEMEncapsulation, operation::KEM_Encapsulate>;
template class ExecutorBase<component::SharedSecret, operation::KEM_Decapsulate>;
template class ExecutorBase<component::PQSignatureKeyPair, operation::PQSIG_KeyGen>;
template class ExecutorBase<component::PQSignature, operation::PQSIG_Sign>;
template class ExecutorBase<bool, operation::PQSIG_Verify>;
