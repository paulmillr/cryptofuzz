...
KEMKeyPair::KEMKeyPair(Datasource& ds) :
    publicKey(ds),
    secretKey(ds)
{ }

KEMKeyPair::KEMKeyPair(PQPublicKey publicKey, PQSecretKey secretKey) :
    publicKey(std::move(publicKey)),
    secretKey(std::move(secretKey))
{ }

KEMKeyPair::KEMKeyPair(nlohmann::json json) :
    publicKey(json["publicKey"]),
    secretKey(json["secretKey"])
{ }

bool KEMKeyPair::operator==(const KEMKeyPair& rhs) const {
    return publicKey == rhs.publicKey && secretKey == rhs.secretKey;
}

void KEMKeyPair::Serialize(Datasource& ds) const {
    publicKey.Serialize(ds);
    secretKey.Serialize(ds);
}

nlohmann::json KEMKeyPair::ToJSON(void) const {
    return {
...
KEMEncapsulation::KEMEncapsulation(Datasource& ds) :
    ciphertext(ds),
    sharedSecret(ds)
{ }

KEMEncapsulation::KEMEncapsulation(KEMCiphertext ciphertext, SharedSecret sharedSecret) :
    ciphertext(std::move(ciphertext)),
    sharedSecret(std::move(sharedSecret))
{ }

KEMEncapsulation::KEMEncapsulation(nlohmann::json json) :
    ciphertext(json["ciphertext"]),
    sharedSecret(json["sharedSecret"])
{ }

bool KEMEncapsulation::operator==(const KEMEncapsulation& rhs) const {
    return ciphertext == rhs.ciphertext && sharedSecret == rhs.sharedSecret;
}

void KEMEncapsulation::Serialize(Datasource& ds) const {
    ciphertext.Serialize(ds);
    sharedSecret.Serialize(ds);
}

nlohmann::json KEMEncapsulation::ToJSON(void) const {
    return {
...
PQSignatureKeyPair::PQSignatureKeyPair(Datasource& ds) :
    publicKey(ds),
    secretKey(ds)
{ }

PQSignatureKeyPair::PQSignatureKeyPair(PQPublicKey publicKey, PQSecretKey secretKey) :
    publicKey(std::move(publicKey)),
    secretKey(std::move(secretKey))
{ }

PQSignatureKeyPair::PQSignatureKeyPair(nlohmann::json json) :
    publicKey(json["publicKey"]),
    secretKey(json["secretKey"])
{ }

bool PQSignatureKeyPair::operator==(const PQSignatureKeyPair& rhs) const {
    return publicKey == rhs.publicKey && secretKey == rhs.secretKey;
}

void PQSignatureKeyPair::Serialize(Datasource& ds) const {
    publicKey.Serialize(ds);
    secretKey.Serialize(ds);
}

nlohmann::json PQSignatureKeyPair::ToJSON(void) const {
    return {
        {"publicKey", publicKey.ToJSON()},
        {"secretKey", secretKey.ToJSON()},
    };
