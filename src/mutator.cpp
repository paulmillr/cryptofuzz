...
static uint32_t prngState = 5323;
static std::default_random_engine rng {};

uint32_t PRNG(void)
{
    prngState = (8253729 * prngState + 2396403);
    return prngState % 32767;
}

static void SeedPRNG(const unsigned int seed) {
    prngState = seed;
    rng.seed(seed);
}
...
static uint64_t getRandomKEM(void) {
    return KEMLUT[ PRNG() % (sizeof(KEMLUT) / sizeof(KEMLUT[0])) ].id;
}
...
static size_t getKEMSeedSize(const uint64_t kemID) {
    switch ( kemID ) {
        case CF_KEM("X-Wing"):
        case CF_KEM("ML-KEM-1024-P384"):
        case CF_KEM("QSF-ML-KEM-1024-P384"):
        case CF_KEM("KitchenSink-ML-KEM-768-X25519"):
        case CF_KEM("ML-KEM-768-P256"):
        case CF_KEM("ML-KEM-768-X25519"):
        case CF_KEM("QSF-ML-KEM-768-P256"):
            return 32;
        default:
            return 64;
    }
...
static size_t getKEMCoinsSize(const uint64_t kemID) {
    switch ( kemID ) {
        case CF_KEM("QSF-ML-KEM-1024-P384"):
            return 104;
        case CF_KEM("ML-KEM-768-P256"):
...
static size_t getPQSignatureSeedSize(const uint64_t pqSignatureID) {
    switch ( pqSignatureID ) {
        case CF_PQSIG("Falcon-1024"):
        case CF_PQSIG("Falcon-512"):
            return 48;
        case CF_PQSIG("SLH-DSA-SHA2-128f"):
        case CF_PQSIG("SLH-DSA-SHA2-128s"):
        case CF_PQSIG("SLH-DSA-SHAKE-128f"):
        case CF_PQSIG("SLH-DSA-SHAKE-128s"):
            return 48;
        case CF_PQSIG("SLH-DSA-SHA2-192f"):
        case CF_PQSIG("SLH-DSA-SHA2-192s"):
        case CF_PQSIG("SLH-DSA-SHAKE-192f"):
        case CF_PQSIG("SLH-DSA-SHAKE-192s"):
            return 72;
        case CF_PQSIG("SLH-DSA-SHA2-256f"):
        case CF_PQSIG("SLH-DSA-SHA2-256s"):
        case CF_PQSIG("SLH-DSA-SHAKE-256f"):
        case CF_PQSIG("SLH-DSA-SHAKE-256s"):
            return 96;
        default:
            return 32;
    }
...
static size_t getPQSignatureExtraEntropySize(const uint64_t pqSignatureID) {
    switch ( pqSignatureID ) {
        case CF_PQSIG("Falcon-1024"):
        case CF_PQSIG("Falcon-512"):
            return 48;
...
#define GET_OR_BIGNUM(x) getBool() ? (x) : getBignum();
        switch ( operation ) {
            ...
            case    CF_OPERATION("SymmetricDecrypt"):
                ...
                break;
            case    CF_OPERATION("KEM_KeyGen"):
                {
                    const auto kemID = getRandomKEM();
                    parameters["modifier"] = getBuffer(PRNG() % 256);
                    parameters["kemType"] = kemID;
                    parameters["seed"] = getBuffer(getKEMSeedSize(kemID));

                    cryptofuzz::operation::KEM_KeyGen op(parameters);
                    op.Serialize(dsOut2);
                }
                break;
            case    CF_OPERATION("KEM_Encapsulate"):
                {
                    uint64_t kemID;
                    if ( Pool_KEMKeyPair.Have() ) {
                        const auto keyPair = Pool_KEMKeyPair.Get();
                        kemID = keyPair.kemID;
                        parameters["publicKey"] = (PRNG() % 8) == 0 ?
                            getBuffer(PRNG() % 256) : keyPair.publicKey;
                    } else {
                        kemID = getRandomKEM();
                        parameters["publicKey"] = getBuffer(PRNG() % 4096);
                    }

                    parameters["modifier"] = getBuffer(PRNG() % 256);
                    parameters["kemType"] = kemID;
                    parameters["coins"] = getBuffer(getKEMCoinsSize(kemID));
            ...
            case    CF_OPERATION("KEM_Decapsulate"):
                {
                    if ( Pool_KEMEncapsulation.Have() ) {
                        const auto encapsulation = Pool_KEMEncapsulation.Get();
                        parameters["kemType"] = encapsulation.kemID;
                        parameters["secretKey"] = (PRNG() % 8) == 0 ?
                            getBuffer(PRNG() % 4096) : encapsulation.secretKey;
                        parameters["ciphertext"] = (PRNG() % 8) == 0 ?
                            getBuffer(PRNG() % 4096) : encapsulation.ciphertext;
                    } else {
                        parameters["kemType"] = getRandomKEM();
                        parameters["secretKey"] = getBuffer(PRNG() % 4096);
                        parameters["ciphertext"] = getBuffer(PRNG() % 4096);
                    }
                    parameters["modifier"] = getBuffer(PRNG() % 256);
                ...
                break;
            case    CF_OPERATION("PQSIG_KeyGen"):
                {
                    const auto pqSignatureID = getRandomPQSignature();
                    parameters["modifier"] = getBuffer(PRNG() % 256);
                    parameters["pqSignatureType"] = pqSignatureID;
                    parameters["seed"] = getBuffer(getPQSignatureSeedSize(pqSignatureID));

                    cryptofuzz::operation::PQSIG_KeyGen op(parameters);
                    op.Serialize(dsOut2);
                }
                break;
            case    CF_OPERATION("PQSIG_Sign"):
                {
                    uint64_t pqSignatureID;
                    if ( Pool_PQSignatureKeyPair.Have() ) {
                        const auto keyPair = Pool_PQSignatureKeyPair.Get();
                        pqSignatureID = keyPair.pqSignatureID;
                        parameters["secretKey"] = (PRNG() % 8) == 0 ?
                            getBuffer(PRNG() % 4096) : keyPair.secretKey;
                    } else {
                        pqSignatureID = getRandomPQSignature();
                        parameters["secretKey"] = getBuffer(PRNG() % 4096);
                    }

                    parameters["modifier"] = getBuffer(PRNG() % 256);
                    parameters["pqSignatureType"] = pqSignatureID;
                    parameters["message"] = getBuffer(PRNG() % 1024);
                    parameters["context"] =
                        (pqSignatureID == CF_PQSIG("Falcon-512") ||
                         pqSignatureID == CF_PQSIG("Falcon-1024") ||
                         (PRNG() % 4) != 0) ? "" : getBuffer(PRNG() % 256);
                    parameters["extraEntropy"] = (PRNG() % 2) == 0 ? "" :
                        getBuffer(getPQSignatureExtraEntropySize(pqSignatureID));

                    cryptofuzz::operation::PQSIG_Sign op(parameters);
                    op.Serialize(dsOut2);
                }
                break;
            case    CF_OPERATION("PQSIG_Verify"):
                {
                    if ( Pool_PQSignature.Have() ) {
                        const auto signature = Pool_PQSignature.Get();
                        parameters["pqSignatureType"] = signature.pqSignatureID;
                        parameters["publicKey"] = (PRNG() % 8) == 0 ?
                            getBuffer(PRNG() % 4096) : signature.publicKey;
                        parameters["message"] = (PRNG() % 8) == 0 ?
                            getBuffer(PRNG() % 1024) : signature.message;
                        parameters["signature"] = (PRNG() % 8) == 0 ?
                            getBuffer(PRNG() % 4096) : signature.signature;
                        parameters["context"] = signature.context;
                    } else {
                        parameters["pqSignatureType"] = getRandomPQSignature();
                        parameters["publicKey"] = getBuffer(PRNG() % 4096);
                        parameters["message"] = getBuffer(PRNG() % 1024);
                    ...
                    cryptofuzz::operation::PQSIG_Verify op(parameters);
                    op.Serialize(dsOut2);
                }
                break;
