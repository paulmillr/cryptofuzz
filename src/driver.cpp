...
void Driver::Run(const uint8_t* data, const size_t size) const {
    ...
    static ExecutorKEM_KeyGen executorKEM_KeyGen(CF_OPERATION("KEM_KeyGen"), modules, options);
    ...
    static ExecutorPQSIG_KeyGen executorPQSIG_KeyGen(CF_OPERATION("PQSIG_KeyGen"), modules, options);
    static ExecutorPQSIG_Sign executorPQSIG_Sign(CF_OPERATION("PQSIG_Sign"), modules, options);
    static ExecutorPQSIG_Verify executorPQSIG_Verify(CF_OPERATION("PQSIG_Verify"), modules, options);
    ...
    try {
        ...
        switch ( operation ) {
            ...
            case CF_OPERATION("KEM_KeyGen"):
                executorKEM_KeyGen.Run(ds, payload.data(), payload.size());
                break;
            ...
            case CF_OPERATION("PQSIG_KeyGen"):
                executorPQSIG_KeyGen.Run(ds, payload.data(), payload.size());
                break;
            case CF_OPERATION("PQSIG_Sign"):
                executorPQSIG_Sign.Run(ds, payload.data(), payload.size());
                break;
            case CF_OPERATION("PQSIG_Verify"):
                executorPQSIG_Verify.Run(ds, payload.data(), payload.size());
                break;
