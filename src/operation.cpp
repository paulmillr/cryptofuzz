...
std::string KEM_KeyGen::Name(void) const { return "KEM_KeyGen"; }
std::string KEM_KeyGen::ToString(void) const {
    std::stringstream ss;
    ss << "operation name: KEM_KeyGen" << std::endl;
    ss << "KEM: " << repository::KEMToString(kemType.Get()) << std::endl;
    ss << "seed: " << util::HexDump(seed.Get()) << std::endl;
    return ss.str();
}

nlohmann::json KEM_KeyGen::ToJSON(void) const {
    return {
        {"operation", "KEM_KeyGen"},
        {"kemType", kemType.ToJSON()},
        {"seed", seed.ToJSON()},
        {"modifier", modifier.ToJSON()},
    };
}

std::string KEM_Encapsulate::Name(void) const { return "KEM_Encapsulate"; }
std::string KEM_Encapsulate::ToString(void) const {
    std::stringstream ss;
    ss << "operation name: KEM_Encapsulate" << std::endl;
    ss << "KEM: " << repository::KEMToString(kemType.Get()) << std::endl;
    ss << "public key: " << util::HexDump(publicKey.Get()) << std::endl;
    ss << "coins: " << util::HexDump(coins.Get()) << std::endl;
    return ss.str();
}
...
std::string KEM_Decapsulate::Name(void) const { return "KEM_Decapsulate"; }
std::string KEM_Decapsulate::ToString(void) const {
    std::stringstream ss;
    ss << "operation name: KEM_Decapsulate" << std::endl;
    ss << "KEM: " << repository::KEMToString(kemType.Get()) << std::endl;
    ss << "secret key: " << util::HexDump(secretKey.Get()) << std::endl;
    ss << "ciphertext: " << util::HexDump(ciphertext.Get()) << std::endl;
    return ss.str();
}
...
std::string PQSIG_KeyGen::Name(void) const { return "PQSIG_KeyGen"; }
std::string PQSIG_KeyGen::ToString(void) const {
    std::stringstream ss;
    ss << "operation name: PQSIG_KeyGen" << std::endl;
    ss << "signature algorithm: " << repository::PQSIGToString(pqSignatureType.Get()) << std::endl;
    ss << "seed: " << util::HexDump(seed.Get()) << std::endl;
    return ss.str();
}

nlohmann::json PQSIG_KeyGen::ToJSON(void) const {
    return {
        {"operation", "PQSIG_KeyGen"},
        {"pqSignatureType", pqSignatureType.ToJSON()},
        {"seed", seed.ToJSON()},
        {"modifier", modifier.ToJSON()},
    };
}

std::string PQSIG_Sign::Name(void) const { return "PQSIG_Sign"; }
std::string PQSIG_Sign::ToString(void) const {
    std::stringstream ss;
    ss << "operation name: PQSIG_Sign" << std::endl;
    ss << "signature algorithm: " << repository::PQSIGToString(pqSignatureType.Get()) << std::endl;
    ss << "secret key: " << util::HexDump(secretKey.Get()) << std::endl;
    ss << "message: " << util::HexDump(message.Get()) << std::endl;
    ss << "context: " << util::HexDump(context.Get()) << std::endl;
    ss << "extra entropy: " << util::HexDump(extraEntropy.Get()) << std::endl;
    return ss.str();
}

nlohmann::json PQSIG_Sign::ToJSON(void) const {
    return {
        {"operation", "PQSIG_Sign"},
        {"pqSignatureType", pqSignatureType.ToJSON()},
        {"secretKey", secretKey.ToJSON()},
        {"message", message.ToJSON()},
        {"context", context.ToJSON()},
        {"extraEntropy", extraEntropy.ToJSON()},
        {"modifier", modifier.ToJSON()},
    };
}

std::string PQSIG_Verify::Name(void) const { return "PQSIG_Verify"; }
std::string PQSIG_Verify::ToString(void) const {
    std::stringstream ss;
    ss << "operation name: PQSIG_Verify" << std::endl;
    ss << "signature algorithm: " << repository::PQSIGToString(pqSignatureType.Get()) << std::endl;
    ss << "public key: " << util::HexDump(publicKey.Get()) << std::endl;
    ss << "message: " << util::HexDump(message.Get()) << std::endl;
...
nlohmann::json PQSIG_Verify::ToJSON(void) const {
    return {
        {"operation", "PQSIG_Verify"},
        {"pqSignatureType", pqSignatureType.ToJSON()},
        {"publicKey", publicKey.ToJSON()},
        {"message", message.ToJSON()},
