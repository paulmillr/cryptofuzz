...
std::string ToString(const component::KEMKeyPair& val) {
    return
        HexDump(val.publicKey.Get(), "public key") + "\n" +
        HexDump(val.secretKey.Get(), "secret key");
}

std::string ToString(const component::KEMEncapsulation& val) {
    return
        HexDump(val.ciphertext.Get(), "ciphertext") + "\n" +
        HexDump(val.sharedSecret.Get(), "shared secret");
}

std::string ToString(const component::PQSignatureKeyPair& val) {
    return
        HexDump(val.publicKey.Get(), "public key") + "\n" +
        HexDump(val.secretKey.Get(), "secret key");
}
...
nlohmann::json ToJSON(const component::KEMKeyPair& val) {
    return val.ToJSON();
}

nlohmann::json ToJSON(const component::KEMEncapsulation& val) {
    return val.ToJSON();
}

nlohmann::json ToJSON(const component::PQSignatureKeyPair& val) {
    return val.ToJSON();
}
